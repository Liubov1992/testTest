       ...
       $.ajax({
            ...
            catchPhrase: $('.form').find('#catchPhrase').val(),
                    ...
                    $('.form').submit( function(e) {
                        e.preventDefault();
                        var k = Users.length+1;
                        var n = k+1;
                                for (k; k < n; k++){
                                Users.push(newUser);
                                $('#container').append('<tr><td class="id">'+i+'</td><td>'+newUser.name+'</td><td>'+newUser.userName+'</td><td>'+newUser.email+'</td><td>Street: '+newUser.street+'<br>Suite: '+newUser.suite+'<br>City: '+newUser.city+'<br>Zipcode: '+newUser.zipcode+'</td><td>'+newUser.phone+'</td><td>'+newUser.website+'</td><td>Name: '+newUser.companyName+'<br>Catch phrase: '+newUser.catchPhrase+'</td></tr>');  
